...
class Unit < Numeric
    ...
    return self if self.is_base?
     if self.units =~ /\A(deg|temp)(C|F|K|C)\Z/
      ...
      base = case self.units
           when /temp/ then self.to('tempK')
           when /deg/ then self.to('degK')
      end
      return base
    end
  ...
  def to_f(target_unit =nil)
    return @scalar.to_f if self.unitless? && target_unit.nil
    if target_unit
       return self.to(target_unit).scalar
    end
