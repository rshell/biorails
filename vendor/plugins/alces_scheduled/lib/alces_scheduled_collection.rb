...
module Alces
  ...
  module ScheduledCollection #:nodoc:
    DEFAULT_SCHEDULE_SUMMARY = <<SQL
          case 
          when states.level_no in (0,1,2) and ended_at < current_date then 'overdue'
          when states.level_no in (0,1,2) and ended_at > current_date then 'todo'
          when states.level_no < 0  then 'failed'
          else 'done'
          end
SQL
    ...
    DEFAULT_SCHEDULE_ORDER = <<SQL
          case 
          when states.level_no in (0,1,2) and ended_at < current_date then '1'
          when states.level_no in (0,1,2) and ended_at > current_date then '2'
          when states.level_no < 0  then '3'
          else '4'
          end, ended_at
SQL
    ...
    module ClassMethods
      ...
      def has_many_scheduled(scheduled_collection = :schedule, options = {})
        ...
        class_eval do
          ...
          has_many scheduled_collection ,{:include=>[:project_element=>[:state]],:order =>"started_at asc"}.merge(options) do
            ##
            ...
            def live(options={})
              with_scope :find => options  do
                find(:all,:conditions=>'states.level_no in (1,2)', :order=> DEFAULT_SCHEDULE_ORDER )
              end
            end
            #
            ...
            def todo(options={})
              ...
              with_scope :find => options  do
                find(:all,:conditions=>'states.level_no in (0,1)', :order=> DEFAULT_SCHEDULE_ORDER )
              end
            end
            ##
            ...
            def overdue(limit=10,options={})
              with_scope :find => options  do
                find(:all,:conditions=>["#{self.proxy_reflection.klass.table_name}.expected_at < ? and states.level_no in (0,1,2) ",Time.now],:limit=>limit  )
              end
            end
            ##
            ...
            def current(limit=10,options={})
              with_scope :find => options  do
                find(:all,
                  :conditions=>["? between #{self.proxy_reflection.klass.table_name}.started_at and #{self.proxy_reflection.klass.table_name}.expected_at and states.level_no in (0,1,2) ",Time.now],
                  :limit=>limit  )
              end
            end
            ...
            def count_status(object)
              case object
              when Array
                count(:conditions=>"states.level_no in (#{object.join(',')})" )
              else
                count(:conditions=>["states.level_no = ?",object])
              end
            end
            ...
            def future(limit=10,options={})
              with_scope :find => options do
                find(:all,
                  :conditions=>[" #{self.proxy_reflection.klass.table_name}.started_at > ?  and states.level_no in (0,1,2) ",Time.now],
                  :limit=>limit )
              end
            end
            ...
            def status_summary
              out =""
              out << "/overdue:#{count(:conditions=>['states.level_no in (1,2) and expected_at < ?',Time.new])}"
              out << "/todo:#{   count(:conditions=>['states.level_no in (1,2) and expected_at >= ?',Time.new])}"
              out << "/failed:#{ count(:conditions=>['states.level_no  < 0'])}"
              out << "/done:#{   count(:conditions=>['states.level_no  > 2'])}"
              return out
            end
                
            def summary
              return "#{count_status(0)} / #{count_status([1,2])} / #{count_status([3,4,-1])}"
            end
          end
