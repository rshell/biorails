    ...
    module NestedSet #:nodoc:
      ...
      module ClassMethods                
        ...
        def acts_as_fast_nested_set(options = {})          
          ...
          class_eval <<-EOV
            ...
            def self.roots
              find(:all, :conditions => "#{configuration[:parent_column]} IS NULL", :order => #{configuration[:order].nil? ? "nil" : %Q{"#{configuration[:order]}"}})
            end

            def self.root
              find(:first, :conditions => "#{configuration[:parent_column]} IS NULL", :order => #{configuration[:order].nil? ? "nil" : %Q{"#{configuration[:order]}"}})
            end
      ...
      module InstanceMethods
        ...
        def ancestors
            ...
            :conditions => ["#{scope_condition} AND #{left_col_name} < ? and #{right_col_name} > ? ",
       ...
       #
        ...
        def add_child( child )     
          ...
          if ( (self[left_col_name] == nil) || (self[right_col_name] == nil) || (self[left_col_name] == 0) || (self[right_col_name] == 0) )
