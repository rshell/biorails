...
module CalendarHelper
  ...
  def month_navigator( schedule,options ={'action'=>'show'})
    out = '<table width="100%">'
  ...
  def calendar(schedule , &proc)
    ...
    while day <= schedule.last_date
      ...
      out <<  (schedule.in_range?(day) ? " ":" grey") 
      out <<  (Date.today == day ?  " today" : "" ) 
      out << "'> " 
      out << " <p class='day'>"  
      out << ( day==Date.today ? "<b>#{day.day}</b>" : day.day.to_s)   
      out << " </p>"	
      for item in schedule.for_day(day) 
        out << "<div class='tooltip'> <p class='state_#{item.status}'>"
        out << image_tag("state/icon-status_#{item.status}.png") if item.starting?(day)
        if block_given?    
          out << capture(item,&proc)  if item.starting?(day)
        else
          out << item.name  if item.starting?(day)
        end
...
#Note to Bob
# I have removed the end date markers as thyey clutter the calendar up and its the start that is important.
# Replaced the start with a status flag too
# Perhaps a good idea to have start and ednd dates s a filter option but thats a bit beyond me.
