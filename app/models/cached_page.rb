...
class CachedPage < ActiveRecord::Base
  ...
  class << self
    ...
    def expire_pages(project, pages)
      update_all ['cleared_at = ?', Time.now.utc], ["project_id = ? and id IN (?)", project.id, pages.collect(&:id)] unless pages.empty?
    end
    
    def create_by_url(project, url, references)
      returning find_or_initialize_by_project_id_and_url(project.id, url) do |page|
