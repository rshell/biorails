class Asset < ActiveRecord::Base
  ...
  @@other_condition = send(:sanitize_sql, ['content_type NOT LIKE ? AND content_type NOT LIKE ? AND content_type NOT IN (?)',
  ...
  def public_filename_with_host(thumbnail = nil)
    returning public_filename_without_host(thumbnail) do |s|
      s.gsub! /^\/assets\/[^\/]+\//, "/assets/#{$1}" if Project.multi_projects_enabled
    end
  end
  ...
  protected
    def rename_unique_filename
      if (@old_filename || new_record?) && errors.empty? && project_id && filename
        i      = 1
    ...
    def permalink
      ...
      pieces = [project.host, date.year, date.month, date.day]
      pieces.shift unless Site.multi_projects_enabled
      pieces * '/'
    end
    
    def set_project_from_parent
      self.project_id = parent.project_id if parent_id
    end
end
