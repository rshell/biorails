...
#  title              :string(255)
#  comments           :string(255)
#  time_source        :integer(4)
#  content_hash       :string(2048)
#  created_at         :datetime
#  created_by_user_id :integer(4)
#  updated_at         :datetime
#  updated_by_user_id :integer(4)
#  file_version       :integer(4)      default(0), not null
#

# == Description
# This represents a electronic signature of a project element
# 
# This will be linked into the current ProjectElement and its children
#
#    * A Project Element can have multiple signatures
#    * Each signature linked to a specific Asset/Article Version
#    * Signatures are linked to checksums held on at the asset/article version label
#    * The checksum is based on the contents of key display fields
#    * The signature container a are specific to a user and time
#    * A pluggable countersign procedure is called to perform signatures
#    * A pluggable checksum procedure is called to generate checksums 
#    * Signature may have multiple child components
#    * For ProjectReferences (Study,Experiment etc) a copy of the current state is save in XML->asset and HTML->article for signing 
#
# == Copyright
# 
...
class Signature < ActiveRecord::Base
  ...
  def verify_the_digest(text=text_to_sign)
    unless text.nil?
      hash = Base64.decode64(content_hash)
      get_public_key.verify(Signature.hashfunc,hash, text)
    else
      nil
    end
  end
  ...
  # 
  def file_url
    File.join("",SystemSetting.published_folder,project_element.project.dom_id,"#{self.filename}.pdf")
  end

  def zip_url
    File.join("",SystemSetting.published_folder,project_element.project.dom_id,"#{self.filename}.zip")
  end
  
  def create_sig
    sig_file=File.open("#{self.file_full_path}_#{self.signature_role.downcase}_signature.xml","w")
    sig_file.write(self.to_xml())
    sig_file.close
    save!
    make_zip(File.join(project_element.folder_filepath,"..",self.filename) + '.zip')
    return self.id
  end
  #
  ...
  #
  def make_files_for_signing
    logger.info "making pdf to sign"
    return false unless self.project_element
    filename = File.join("/tmp","#{project_element.dom_id}.pdf")  
    self.project_element.make_pdf(filename)
    #
    ...
    begin
      self.file_version += 1
      self.filename =  "#{self.project_element.dom_id}_V#{self.file_version+=1}"
      pdf_filename = File.join(project_element.folder_filepath,"..",self.filename) + '.pdf'
    end while File.exists?(pdf_filename)    
...
# 
  ...
  def make_zip(destination)
     ...
     # collect the album's tracks
      zipfile.add("#{self.filename}.pdf",File.join(project_element.folder_filepath,"..",self.filename) + '.pdf')
      Dir.foreach(project_element.folder_filepath) do |entry|
         path = File.join(project_element.folder_filepath,entry)
         if File.file?(path)
