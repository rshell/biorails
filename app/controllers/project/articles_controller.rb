class Project::ArticlesController < ApplicationController
  ...
  before_filter :find_project_article, :only => [:edit, :update, :comments, :approve, :unapprove, :destroy]

  before_filter :load_sections, :only => [:new, :edit]

  def index
    @article_pages = Paginator.new self, current_project.articles.count(:all, article_options), 30, params[:page]
    @articles      = current_project.articles.find(:all, article_options(:order => 'contents.published_at DESC', :select => 'contents.*', 
                       :limit   =>  @article_pages.items_per_page,
                       :offset  =>  @article_pages.current.offset))
    
    @comments = current_project.unapproved_comments.count :all, :group => :article, :order => '1 desc'
    @sections = current_project.sections.find(:all)
  end


  def show
    @article  = current_project.articles.find(params[:id])
    @comments = @article.comments.collect &:to_liquid
    Biorails::Liquid::CommentForm.article = @article
    @article  = @article.to_liquid(:mode => :single)
    
    render :text => current_project.render_liquid_for(current_project.sections.home, :single, 'articles' => [@article], 'article' => @article, 'comments' => @comments, 'site' => current_project.to_liquid)
  end

  def new
    @article = current_project.articles.build(:comment_age => current_project.comment_age, :filter => current_user.filter, :published_at => utc_to_local(Time.now.utc))
  end
  ...
  def create
    @article = current_user.articles.create params[:article].merge(:updater => current_user, :project => project)
  ...
  def comments
    ...
    @articles = current_project.unapproved_comments.count :all, :group => :article, :order => '1 desc'
  end
  ...
  def destroy_comment
    @comments = current_project.all_comments.find :all, :conditions => ['id in (?)', [params[:comment]].flatten] rescue []
    Comment.transaction { @comments.each(&:destroy) } if @comments.any?
  end

  def upload
    @asset   = current_project.assets.build(params[:asset])
    @asset.save!
  rescue ActiveRecord::RecordInvalid
  ensure
  ...
  protected
    def load_sections
      @assets = current_project.assets.find(:all, :order => 'created_at desc', :limit => 6)
      @sections = current_project.sections.find(:all)
