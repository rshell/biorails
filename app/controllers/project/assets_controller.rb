...
#  before_filter :find_asset, :except => [:index, :new, :create, :latest, :search, :upload, :clear_bucket]
  ...
  protected
    ...
    def search_assets(limit)
      ...
      @assets = @types.any? ?
        current_project.assets.find_all_by_content_types(@types, :all, search_options) :
        current_project.assets.find(:all, search_options)
    end
    ...
    def count_by_conditions
      ...
      @count_by_conditions ||= search_conditions[:conditions].blank? ? current_project.assets.count(:all, :conditions => type_conditions) :
        Asset.count( 
