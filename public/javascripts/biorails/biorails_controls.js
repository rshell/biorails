    ...
    save: function(){
        if (this.url && (_original_value != this.getValue())) {
            ...
            new Ajax.Request(save_url, {
                asynchronous: true,
                evalScripts: true,
                parameters: 'value=' + escape(element.value)
            });
        }
    },
    ...
    Biorails.ComboField.superclass.constructor.call(this, Ext.apply(config, {
        ...
        store: new Ext.data.Store({
            proxy: new Ext.data.HttpProxy({
                url: '/admin/element/select/' + config.root_id,
                method: 'post'
            }),
...
Biorails.RegExpField = function(config){
    Biorails.RegExpField.superclass.constructor.call(this, Ext.apply(config, {
        regexText: 'Failed to meet mask'
    }));
...
Biorails.FileComboField = function(config){
    ...
    Biorails.FileComboField.superclass.constructor.call(this, Ext.apply(config, {
        ...
        store: new Ext.data.Store({
            proxy: new Ext.data.HttpProxy({
                url: '/folders/select/' + config.folder_id,
                method: 'post'
            }),
            reader: new Ext.data.JsonReader({
                root: 'items',
...
Biorails.ConceptTree = function(el){
    ...
    Biorails.ConceptTree.superclass.constructor.call(this, {
        ...
        loader: new Ext.tree.TreeLoader({
            dataUrl: '/admin/catalogue/tree'
        })
    });
    
    this.on('click', function(node){
        try {
            new Ajax.Request(node.attributes.url, {
                asynchronous: true,
                evalScripts: true
            });
        } 
...
Ext.namespace("Biorails.Dialog");

Biorails.Dialog = function(config){
    var button = Ext.get(config.button);
    var win = null;
    button.on('click', function(){
        // create the window on the first click and reuse on subsequent clicks
        if (!win) {
            win = new Ext.Window(Ext.apply(config, {
