...
class StudyParameterTest < Test::Unit::TestCase
  fixtures :study_parameters
	...
	NEW_STUDY_PARAMETER = {}	# e.g. {:name => 'Test StudyParameter', :description => 'Dummy'}
  ...
  def test_raw_validation
    ...
    if REQ_ATTR_NAMES.blank?
      assert study_parameter.valid?, "StudyParameter should be valid without initialisation parameters"
    else
      # If StudyParameter has validation, then use the following:
      assert !study_parameter.valid?, "StudyParameter should not be valid without initialisation parameters"
      REQ_ATTR_NAMES.each {|attr_name| assert study_parameter.errors.invalid?(attr_name.to_sym), "Should be an error message for :#{attr_name}"}
    end
  end
	...
	def test_duplicate
    current_study_parameter = StudyParameter.find_first
