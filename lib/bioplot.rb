...
require 'matrix'
 ##
 # This is a branch of pgplot  I adding a number of  domain specfic plot types too
 # for fitting etc.
 #
module BioPlot
  ...
  def BioPlot.gnuplot( persist = true )
  ...
  class Plot
    attr_accessor :cmd, :data, :sets,:variables
   ...
   def var(var,value = "")
    ...
    def to_gplot (io = "")
      ...
      @variables.each { |var, val| io << " #{var} = #{val}\n" }
  ...
  ##
  # Perform a Function fit on the data
  #
 class Fit < Plot

  def initialize (io = nil, cmd = "fit")
      super
    end
...
##
# Add methods to convert array to gnuplot
#
class Array
 ...
 ##
 # extend the Matrix class to add a convert  to gnuplot  function
 #
class Matrix
