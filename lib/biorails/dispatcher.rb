module Biorails
  class Dispatcher
    ...
    def self.run(project, path)
      # check for any bad urls like /foo//bar
      return [:error, nil, *path] if path.any? &:blank?
      
      if args = Biorails::Routing.handle_redirection(path * "/")
        return [:redirect, *args]
      end

      # check for permalink
      if options = recognize_permalink(project, path)
      ...
      # check for tags
      return [:tags, nil] + path[1..-1] if path.first == project.tag_path
      
      # check for search
      if path.first == project.search_path
      ...
      returning [] do |result|
        ...
        while section.nil? && path.any?
          section = project.sections.detect { |s| s.path == path.join('/') }
          result << path.pop if section.nil?
        end
        section ||= project.sections.home
        result.reverse!
    ...
    def self.recognize_permalink(project, path)
      full_path = path.join('/')
      regex, variables = build_permalink_regex_with(project.permalink_style)
