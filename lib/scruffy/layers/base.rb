module Scruffy::Layers
  ...
  class Base
    ...
    # 
    # The standard svg operations mapped into ruby via the builder xml library. Simply use valid 
    # svg element and attribute names as ruby magic will do the rest. As a handy hints the following
    # svg elements are the most useful.
    # 
    # * circle(cx,cy,r)
    # * elipse(cx,cy,rx,ry)
    # * rect(x,y,height,width,style)
    # * line(x1,y1,x2,y2)
    # * polyline(points)
    # * image()
    # * path()
    # * text(x,y,text,lengh)
    # 
    # See any good tutorial on svg like [http://apike.ca/prog_svg_shapes.html] for details of the options available.
    # Its may also help to look at examples like [http://www.carto.net/papers/svg/samples/]
    # 
    ...
    protected
      ...
      def generate_coordinates(options = {})
        ...
        coords = (0...points.size).map do |idx| 
          puts "#{options[:point_distance] * idx},#{percentage(points[idx])}" 
          [options[:point_distance] * idx , percentage(points[idx])]
        end
      ...
      ##
      # Convert y coordinates to percentage of height
      #
      def percentage(value)
          @height - (@height * ((value == @min_value) ? 0 : ((value - @min_value) / (@max_value - @min_value).to_f)))
      end
